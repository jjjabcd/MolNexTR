...
def train_loop(args, train_df, valid_df, aux_df, tokenizer, save_path):
    ...
    # Load starting epoch and global step from checkpoint
    if args.resume and args.load_path:
    ...
    encoder, decoder = get_model(args, tokenizer, device, load_path=args.load_path if args.resume else None)
    encoder_optimizer, encoder_scheduler, decoder_optimizer, decoder_scheduler = \
        get_optimizer_and_scheduler(args, encoder, decoder, load_path=args.load_path if args.resume else None)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.local_rank == 0:
            ...
            print_rank_0(f"Checkpoint saved at {checkpoint_filename}")
